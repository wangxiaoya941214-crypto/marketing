...
type InsightItem = InsightResult["anomalies"][number];
type InsightGroupKey = "anomalies" | "opportunities" | "risks";
...
const formatInsightValue = (value: InsightItem["value"]) =>
  typeof value === "number" ? `${Math.round(value * 100) / 100}` : value || "——";

const INSIGHT_GROUPS: Array<{ key: InsightGroupKey; label: string }> = [
  { key: "anomalies", label: "异常洞察" },
  { key: "opportunities", label: "机会点" },
...
const getInsightSeverityTone = (severity: InsightItem["severity"]) => {
  if (severity === "high") {
    return {
      label: "高",
      badgeClass: "border-red-200 bg-red-50 text-red-700",
      blockClass: "border-red-200 bg-red-50/60",
    };
  }
  if (severity === "medium") {
    return {
      ...
      blockClass: "border-yellow-200 bg-yellow-50/70",
    };
  }
  return {
    label: "低",
    badgeClass: "border-[#08E03B]/20 bg-[#08E03B]/10 text-[#067b21]",
    blockClass: "border-[#08E03B]/20 bg-[#08E03B]/5",
  };
};
...
const InsightSummaryArticle = ({ insights }: { insights: InsightResult }) => {
  const hasInsightItems =
  ...
  return (
    <article className={`${MODULE_CARD_CLASS} p-6`}>
      <div className="space-y-2">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400">
          完整报告首段
        </p>
        <h3 className="text-2xl font-black tracking-tight text-gray-950">AI 洞察摘要</h3>
        <p className="text-sm font-medium leading-6 text-gray-600">
          这部分先把 AI 洞察层提炼出的重点、异常、机会点和风险预警单独拎出来，方便快速判断优先级。
        </p>
      </div>

      <div className="mt-5 border-t border-gray-100 pt-5">
        {!hasInsightItems ? (
          <div className="rounded-[1.5rem] border border-gray-200 bg-gray-50 px-4 py-4 text-sm font-medium leading-6 text-gray-600">
            本轮未生成可用 AI 洞察，以下为结构化完整报告。
          </div>
        ) : (
          <div className="space-y-6">
            {insights.topFindings.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                  {insights.topFindings.map((item, index) => (
                    <div
                      key={`${item}-${index}`}
                      className="rounded-[1.5rem] border border-[#08E03B]/20 bg-[#08E03B]/5 px-4 py-4"
                    >
                      <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#067b21]">
                        重点 {index + 1}
                      </p>
                      <p className="mt-2 text-sm font-bold leading-6 text-gray-800">{item}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {INSIGHT_GROUPS.map((group) => {
              const items = insights[group.key];
              if (!items.length) {
                return null;
              }
                ...
                <div key={group.key} className="space-y-3">
                  ...
                  <div className="space-y-3">
                    {items.map((item, index) => {
                      const tone = getInsightSeverityTone(item.severity);
                      return (
                        <div
                          key={`${group.key}-${item.title}-${index}`}
                          className={`rounded-[1.5rem] border px-4 py-4 ${tone.blockClass}`}
                        >
                          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <span
                              className={`inline-flex w-fit rounded-full border px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] ${tone.badgeClass}`}
                            >
                              风险等级 {tone.label}
                            </span>
                          </div>
                          ...
                          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                            <div className="rounded-[1.25rem] border border-white/70 bg-white/70 px-4 py-4">
                              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-gray-400">
                                指标
                              </p>
...
export const AnalysisReportSection = ({
  analysis,
  insights,
}: {
  analysis: string;
  ...
  return (
    <section id={SECTION_IDS.analysis} className="scroll-mt-36 space-y-6">
      ...
      <div className="space-y-4">
        <InsightSummaryArticle insights={insights} />
