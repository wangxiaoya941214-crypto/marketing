...
test("渠道分组会区分平台与账号类型 / IP", () => {
  ...
  const normalized = normalizeLeadRows(sheets, {
    kind: "lead_detail_sheet",
    sheetName: "主线索表",
    headerRowIndex: 0,
  ...
  assert.equal(normalized.rows[0]?.channelGroup, "抖音-品牌号");
  assert.equal(normalized.rows[1]?.channelGroup, "抖音-IP号");
  assert.equal(normalized.rows[2]?.channelGroup, "小红书-IP号");
  assert.equal(normalized.rows[3]?.channelGroup, "视频号-IP号");
  assert.equal(normalized.rows[4]?.channelGroup, "私域-老客户");
});
